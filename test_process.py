...
def test_dipole_prelim():
    ...
    if right number of frames is returned by main function assume everything is fine
    """
...
def test_polar_coords_3():
    ...
    ax_1 = np.array([1.,0.,0.])
    ax_2 = np.array([1.,-np.pi/2.,0.])
    res = np.array([1., np.pi/2, np.pi])
    assert np.array_equal(process.polar_coords(test, axis1=ax_1, axis2=ax_2, mod=False), res)
